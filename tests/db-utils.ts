...
export async function cleanupDb(prisma: PrismaClient) {
	const tables = await prisma.$queryRaw<
		{ tablename: string }[]
	>`SELECT tablename::text FROM pg_tables WHERE schemaname = 'public' AND tablename NOT LIKE '_prisma_migrations';`
	...
	try {
		...
		await prisma.$transaction(async (tx) => {
			...
			if (truncateSQL) {
				await tx.$executeRawUnsafe(`TRUNCATE TABLE ${truncateSQL} CASCADE;`)
			}
